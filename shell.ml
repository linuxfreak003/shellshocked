...
let de_empty lst = foldl (fun acc x -> if phys_equal x "" then acc else x::acc) [] lst |> List.rev;;

let tokenize s = String.split_on_chars ~on:[' ';'\t';'\n';'\r'] s |> de_empty;;

let parse_alias lst =
    let str = String.strip (String.concat ~sep:" " lst) in
    let stripped = String.strip ~drop:(fun c -> phys_equal c '"') str in
...
let execute = function
    ...
    | (x::xs) as command ->
        try
            let pid = Unix.fork_exec ~prog:(String.concat " " command) ~use_path:true () in
